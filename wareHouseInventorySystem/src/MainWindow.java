...
public class MainWindow {
	...
	public MainWindow() {
	   ...
	   public void SystemDateDisplay() {
	        

	        dateLbl = new JLabel("System Date: ");
	        add(dateLbl);

	        updateDateLabel(); // To show the initial date
	    }

	    private void add(JLabel dateLbl2) {
		...
		private void updateDateLabel() {
	        ...
	        dateLbl.setText("System Date: " + currentDate);
	    }
	/**
	...
	private void partCombBox() {
		try {
			String part = "select* from parts ORDER BY partname ASC";
			PreparedStatement stmt = con.prepareStatement(part);
			ResultSet rst = stmt.executeQuery();
			while(rst.next()) {
				partBox.addItem(rst.getString("partname"));
			}
		}
		catch(Exception e) {
		}
	...
	**/
	...
	private void initialize() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					String query = "select vehicles_at_lot.stk_number,vehicles_at_lot.year,vehicles_at_lot.make,vehicles_at_lot.model,vehicles_at_lot.engine,vehicles_at_lot.trans,parts.part_id,parts.partname,inventory.shelf,inventory.location,inventory.notes"
							...
							+ " where model = ? and  partname = '"+partBox2.getSelectedItem()+"'order by year";
					PreparedStatement pst = con.prepareStatement(query);
					pst.setString(1, modelTxt.getText());
		...
		lblNewLabel_3.setBounds(82, 127, 134, 20);
